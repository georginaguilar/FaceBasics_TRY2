...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    {
        private static IConnectionFactory connectionFactory = ConfigurationAssembler.CreateConnectionFactory("prueba");
        ...
        public MainWindow()
        {
            ...
            {
                // Create DB if needed
                if (!conn.Run(Query.DbList()).Contains("prueba"))
                    conn.Run(Query.DbCreate("prueba"));

                // Create table if needed
                if (!conn.Run(face_p.Db.TableList()).Contains("cara"))
                    conn.Run(face_p.Db.TableCreate("cara"));

                //// Read all the contents of the table
                foreach (var caras in conn.Run(face_p.Table))

                   //var valor = faceText;
                   var json = new JavaScriptSerializer().Serialize(valor);
                   label1.Content = String.Format("Face:  {0}  ", json);
            }
